...
(function ($) {
	...
	ajaxopts = $.extend({
	    ...
	    type: 'get',
	    dataType: 'json',
	    // error: jsonp request doesn't support error handling
		...
		if (oembedData.error_code && oembedData.error_code == 404) {
		    settings.onError.call(container, externalUrl, embedProvider);
		} else {
		    if (false !== settings.beforeEmbed.call(container, oembedData)) {
    ...
    function initializeProviders() {
        ...
        defaultProvider = getDefaultOEmbedProvider(settings);
    ...
    function getDefaultOEmbedProvider(settings) {
        var url = "http://oohembed.com/oohembed/";
        if (settings.defaultOEmbedProvider == "embed.ly") {
	    if (settings.embedlyKey) {
		url = 'http://api.embed.ly/1/oembed?key=' + settings.embedlyKey + '&';
	    } else {
		url = 'http://api.embed.ly/1/oembed?';
	    }
	}
        return new $.fn.oembed.OEmbedProvider(settings.defaultOEmbedProvider, null, null, url, "callback");
    }
